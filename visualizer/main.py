...
def main():
    ...
    max_fastest = 5000000
    extra_merge_sort_df = extrapolate(fastest_df, "merge sort", max_fastest)
    extra_quick_sort_df = extrapolate(fastest_df, "quick sort", max_fastest)
    extra_radix_sort_df = extrapolate(fastest_df, "radix sort", max_fastest)
    extra_fastest_df = concat(
        [extra_merge_sort_df, extra_quick_sort_df,
         extra_radix_sort_df], ignore_index=True)
